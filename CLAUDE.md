...
- **`NonBillableMatcher`** — compiles the rules once at load (word rules into a single case-insensitive alternation regex, exact words into a list) and answers `IsNonBillable(label)`. `Create(null)` (key absent) returns `Default` — whole-word "lunch"/"break", matching the historical hard-coded behavior; an empty rule list matches nothing; rules with blank words or unknown match modes are skipped. Consumed via `PunchSession.IsNonBillable(TimeBlock)` to exclude blocks from the workday total.
