...
- **`TimeBlock`** — immutable record representing a booked time slot (StartSlot, Length, Label, Ticket). Slots are 0–95 (96 quarter-hours in a day). Ticket is optional (defaults to `""`). Whether a block is non-billable is decided by the session's `NonBillableMatcher`, not by the record itself.
...
- **`TicketEntry`** — immutable record (`Ticket`, `Title`, `FromLog`) for one row of the ticket picker. `FromLog` marks a row derived from the open day's blocks rather than `tickets.txt`; the view uses it to draw the group headers.
- **`TicketCatalog.Build`** — merges the `tickets.txt` rows with any ticket already used on the open day (deduped case-insensitively, titled by the description of the longest block using it, ordered by earliest start slot). Day-log rows come first, then the saved rows in file order. The result is flat and fully selectable, so picker cursor indexing is unaffected.
- **`PunchStorage.LoadSettings`** — reads the manually-maintained `~/.punch/settings.json` (sits alongside the data dir). Returns a `PunchSettings`; a missing or unparseable file yields defaults. Keys: `targetHours` (decimal daily goal in 15-minute increments, default 8; values that aren't a positive multiple of 0.25 fall back to 8), optional `targetHoursByDay` (per-weekday overrides keyed by case-insensitive day name; `0` = day off) — both used for the status-bar percentage — and optional `nonBillable` (label rules marking blocks as non-billable).
- **`PunchSettings`** — JSON DTO for `settings.json`; `GetTargetHours(DayOfWeek)` resolves the per-day override (clamped ≥0; non-quarter-increment values fall back to `TargetHours`) or falls back to `TargetHours`, and the resolved value is threaded through `PunchSession` into `PunchView.RenderStatusBar` (which omits the percentage when the target is 0). `CreateNonBillableMatcher()` builds the `NonBillableMatcher` from the `NonBillable` rules, threaded into `PunchSession` the same way.
- **`NonBillableRule`** — JSON DTO for one `nonBillable` entry: `word` plus a `match` mode, `"word"` (whole-word match anywhere in the label, the default) or `"exact"` (full-label equality, trimmed).
- **`NonBillableMatcher`** — compiles the rules once at load (word rules into a single case-insensitive alternation regex, exact words into a list) and answers `IsNonBillable(label)`. `Create(null)` (key absent) returns `Default` — whole-word "lunch"/"break", matching the historical hard-coded behavior; an empty rule list matches nothing; null entries and rules with blank words or unknown match modes are skipped. Consumed via `PunchSession.IsNonBillable(TimeBlock)` to exclude blocks from the workday total.
...
- **Ticket summary** (`ShowTicketSummary`, F3 or Ctrl+T) — right-panel view summing hours per ticket with Billable/Unbillable subtotals (split on `PunchSession.IsNonBillable`); F3/Ctrl+T/Esc closes.
